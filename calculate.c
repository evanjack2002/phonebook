...
int main(void)
{
    ...
    fprintf(output, "findName() %lf %lf %lf\n", \
            orig_sum_f / 100.0, opt_sum_f / 100.0, opt_hash_sum_f / 100.0);
    fprintf(output, "total %lf %lf %lf", (orig_sum_a + orig_sum_f) / 100.0, \
            (opt_sum_a + opt_sum_f) / 100.0, (opt_hash_sum_a + opt_hash_sum_f) / 100.0);
    fclose(output);
    return 0;
}
